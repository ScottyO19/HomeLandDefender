...
class Design_7_7_FireBullet extends ActorScript
{
	...
	public var _Ammo:Float;
	public function _customEvent_whenThisHearstrigger():Void
	{
		if(((Engine.engine.getGameAttribute("Ammo") : Float) > 0))
		{
			if(_Fired)
			{
				if(!(_BulletCreated.isAlive()))
				{
					Engine.engine.setGameAttribute("Ammo", ((Engine.engine.getGameAttribute("Ammo") : Float) - 1));
					_CanFire = false;
					_Fired = true;
					...
					playSoundOnChannel(getSound(56), 0);
					if(((Engine.engine.getGameAttribute("Ammo") : Float) <= 0))
					{
						_CanFire = false;
						_Fired = true;
